...
module vibe.data.xml;

import std.conv : to;
import std.string : strip, stripRight, stripLeft, split, replace, indexOf, cmp, icmp, isNumeric, toStringz;
...
class XmlNode
{
	...
	private ptrdiff_t readUntil(string xsrc, string delim) {
		...
		ptrdiff_t i = xsrc.indexOf(delim);
	...
	private string getWSToken(ref string input) {
		...
		int i;
		for(i=0; i<input.length && !isWhite(input[i]) && input[i] != '>' && input[i] != '/' && input[i] != '<' && input[i] != '=' && input[i] != '!'; i++){}
		auto ret = input[0 .. i];
		input = input[i .. $];
		if (!ret.length) {
	...
	private void parseAttribute(XmlNode xml, ref string attrstr, string term = null) {
		string ripName(ref string input) {
			int i;
			for(i=0; i < input.length && !isWhite(input[i]) && input[i] != '='; i++){}
			auto ret = input[0 .. i];
			input = input[i .. $];
			return ret;
		}
	...
	XmlNode[] parseXPath(string xpath, bool caseSensitive = false) {
		...
		ptrdiff_t offset = nextnode.indexOf("[");
		if (offset != -1) {
...
/// Convert xml-encoded special characters such as &amp; amp; back to &amp;.
